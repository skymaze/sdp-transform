...
class TestMethods(unittest.TestCase):
    ...
    def test_parse_params_fmtp(self):
        with open("tests/sdps/normal.sdp") as f:
            ...
            self.assertEqual(
                vid_fmtp_1["profile-level-id"],
                "4d0028",
                "video fmtp 0 profile-level-id",
            )
            self.assertEqual(
                vid_fmtp_1["packetization-mode"], 1, "video fmtp 0 packetization-mode"
            )
            self.assertEqual(
                vid_fmtp_1["sprop-parameter-sets"],
    ...
    def test_parse_params_rids(self):
        with open("tests/sdps/simulcast.sdp") as f:
            ...
            self.assertDictEqual(rid_2_params, {"pt": 98}, "video 2nd rid params")
    ...
    def test_parse_image_attributes(self):
        with open("tests/sdps/simulcast.sdp") as f:
            ...
            sdp_dict = parse(sdp)
            video = sdp_dict["media"][1]
            image_attr_1_recv_params = parseImageAttributes(
                video["imageattrs"][0]["attrs2"]
            )
            self.assertListEqual(
                image_attr_1_recv_params,
                [
                    {"x": 1280, "y": 720},
                    {"x": 320, "y": 180},
                    {"x": 160, "y": 90},
    ...
    def test_parse_simulcast_tream_list(self):
        with open("tests/sdps/simulcast.sdp") as f:
            ...
            sdp_dict = parse(sdp)
            video = sdp_dict["media"][1]
            simulcast_send_streams = parseSimulcastStreamList(
                video["simulcast"]["list1"]
            )
            self.assertListEqual(
                simulcast_send_streams,
                [
            ...
            simulcast_recv_streams = parseSimulcastStreamList(
                video["simulcast"]["list2"]
            )
            self.assertListEqual(
                simulcast_recv_streams,
                [[{"scid": "c", "paused": False}]],
